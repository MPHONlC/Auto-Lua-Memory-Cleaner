...
local HUD_SCENE_NAMES = { hud = true, hudui = true, gamepad_hud = true }
...
function ALC.toggle_core_events()
	if ALC.settings.is_enabled then
		...
		if IsConsoleUI() then
			EVENT_MANAGER:RegisterForEvent(ALC.name .. "_LowMem", EVENT_LUA_LOW_MEMORY, function() ALC.run_manual_cleanup() end)
		end
...
local function alc_run_gc_pass(opts)
	...
	zo_callLater(function()
		...
		if not ALC.settings.is_single_pass_cleanup then collectgarbage("collect") end
